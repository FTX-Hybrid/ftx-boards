...
async function pullAthlete(page, uid, days, tz) {
  return page.evaluate(async ({ uid, days, tz }) => {
    ...
    return {
      ...
      workouts: logged.length,          // logged-workout count in the window
...
async function main() {
  ...
  for (const [name, id] of Object.entries(GROUPS)) {
    console.log(`\n=== ${name} (group ${id}) ===`);
    // ROSTER: probe direct member/roster APIs (authenticated fetch) for this group.
    // Each returns the athlete list without touching the fragile report page.
    const probe = await page.evaluate(async (gid) => {
      const eps = [
        `/api/v4/groups/${gid}/members?fetch_all=true`,
        `/api/v3/groups/${gid}/members?fetch_all=true`,
        `/api/v4/groups/${gid}/users?fetch_all=true`,
        `/api/v3/groups/${gid}/clients?fetch_all=true`,
        `/api/v4/groups/${gid}?fetch_all=true`,
        `/api/v3/groups/${gid}?fetch_all=true`,
        `/api/v4/clients?group_id=${gid}&fetch_all=true`,
        `/api/v3/clients?group_id=${gid}&fetch_all=true`,
        `/api/v4/users?group_id=${gid}&fetch_all=true`,
        `/api/v3/group_memberships?group_id=${gid}&fetch_all=true`,
      ];
      const out = [];
      for (const ep of eps) {
        try {
          ...
          if (r.ok) {
            ...
            arr = Array.isArray(j) ? j : (j && (j.members || j.users || j.clients || j.data || j.results)) || null;
            if (Array.isArray(arr) && arr[0] && typeof arr[0] === 'object') keys = Object.keys(arr[0]).slice(0, 16);
          }
    ...
    // Build the roster from the first endpoint that returned member objects with a name.
    ...
    const rows = !source ? [] : source.map(o => {
      ...
      return { userId: String(uid ?? ''), first: String(first ?? '').trim(), last: String(last ?? '').trim(), logged: 0, time: '' };
    ...
    for (const r of rows) {
      ...
      // performance pull for every roster member (gives the real logged count + detail)
      try {
        const perf = await pullAthlete(page, m.uid, DAYS, TZ);
        if (perf) {
          m.w = perf.workouts;                          // real logged-workout count (window)
